...
NUM_BURNIN = 0
CONDITIONED = True
CURRICULUM = 'blocked' # blocked or randomized

# see experiment for more options
SCH_FPATH="csw_simplest1000.sch"
RFC_FPATH='csw_simplest.rfc'
...
class Exp():
  ...
  def gen_path(self,rfc_cond=None,qpr=QUESTION_PR):
    ...
    while frnode.name != "END":
      ...
      if (random.random() < qpr) and (self.askingQ) and (frnode.name != 'BEGIN'):
  ...
  def gen_k_paths_blocked(self,num_paths,blocking_factor='location.latent',block_len=BLOCK_LEN,qblock_len=10):
    ...
    for path_num in range(num_paths):
      ...
      # if path_num%int(2*qblock_len)==0: 
      #   self.askingQ = False
      # elif path_num%int(qblock_len)==0: 
      #   self.askingQ = True
      # select blocking factor
      block_rfc_cond = bfL[0]
      # print(path_num,'RFC',block_rfc_cond.split('.')[-1],'asking',self.askingQ)
      # generate path and moveon
      # print(path_num,'Q:',self.askingQ)
      # print('cond:',block_rfc_cond)
      path,rfc = self.gen_path(rfc_cond=block_rfc_cond) 
      path_L.append(path)
      rfc_L.append(rfc)
    return path_L,rfc_L

  def gen_k_paths_randomized(self,num_paths,qblock_len=10):
    """ 
    ...
    for path_num in range(num_paths):
      # question blocks
      if path_num%(2*qblock_len) == 0: 
        self.askingQ = True
      elif path_num%(qblock_len) == 0:
        self.askingQ = False
      # generate path and moveon
  ...
  def gen_k_paths(self,num_paths):
    ...
    if CURRICULUM == 'blocked':
      return self.gen_k_paths_blocked(num_paths,block_len=BLOCK_LEN)
    elif CURRICULUM == 'interleaved':
      return self.gen_k_paths_blocked(num_paths,block_len=1)
    elif CURRICULUM == 'randomized':
      return self.gen_k_paths_randomized(num_paths)
